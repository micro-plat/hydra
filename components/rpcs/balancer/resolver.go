...
//NewResolver 返回服务解析器
func NewResolver(plat string, server string, service string, sortPrefix string) ServiceResolver {
	return &Resolver{plat: plat, server: server, service: service, sortPrefix: sortPrefix, closeCh: make([]*Watcher, 0, 2)}
}
...
func (v *Resolver) Resolve(target string) (naming.Watcher, error) {
	...
	rw := &Watcher{client: r, plat: v.plat, server: v.server, service: v.service, sortPrefix: v.sortPrefix, closeCh: make(chan struct{})}
