...
func Test_global_check(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "11.Name-不为空", fields: fields{Name: "/PlatName/SysName/api-mqc/ClusterName"}, errMsg: "平台名称不能为空", wantErr: true, expect: CliFlagObject{RegistryAddr: "zk://192.168.0.1"}, flagVal: CliFlagObject{RegistryAddr: "zk://192.168.0.1"}},
