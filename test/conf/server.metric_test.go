...
func TestMetricNew(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "设置空对象", args: args{host: "", db: "", cron: ""}, want: &metric.Metric{DataBase: "", Cron: ""}},
		{name: "只设置ip对象", args: args{host: "192.168.0.101", db: "", cron: ""}, want: &metric.Metric{Host: "192.168.0.101", DataBase: "", Cron: ""}},
...
func TestMetricGetConf(t *testing.T) {
	...
	test2 := test{name: "metric节点存在,数据错误", cnf: conf.GetAPIConf().GetServerConf(), want: nil, wantErr: true, wantErrStr: "metric配置数据有误"}
	...
	test3 := test{name: "metric节点存在,正确节点", cnf: conf.GetAPIConf().GetServerConf(),
