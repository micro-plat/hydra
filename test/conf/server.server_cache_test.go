...
func Test_cache_Clear(t *testing.T) {
	...
	assert.Equal(t, serveC.GetServerConf().GetVersion(), app.Cache.GetServerCuurVerion("api"), "api serverconf verion Equl")
	assert.Equal(t, oldVerion, app.Cache.GetServerCuurVerion("api"), "当前版本号比对")

	//在添加一个server配置
	serveB := conf.GetAPIConf()
	cuurtVerion := serveB.GetServerConf().GetVersion()
	app.Cache.Save(serveB)
	_, ok := app.Cache.GetServerMaps().Get(fmt.Sprintf("api-%v", oldVerion))
	assert.Equal(t, ok, true, "清除前，旧配置存在")
	assert.Equal(t, cuurtVerion, app.Cache.GetServerCuurVerion("api"), "清除前，当前版本号比对")

	time.Sleep(51 * time.Second)
	_, ok = app.Cache.GetServerMaps().Get(fmt.Sprintf("api-%v", oldVerion))
	assert.Equal(t, ok, false, "清除后，旧配置不存在")
	_, ok = app.Cache.GetServerMaps().Get(fmt.Sprintf("api-%v", cuurtVerion))
	assert.Equal(t, ok, true, "清除后，新配置存在")
	assert.Equal(t, cuurtVerion, app.Cache.GetServerCuurVerion("api"), "清除后，当前版本号比对")
}
