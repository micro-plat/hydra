...
func TestGetConf(t *testing.T) {
	...
	assert.Equal(t, server.GetServerConf().GetRootConf().GetString("address"), ":8081", "端口一致性检查")
	assert.Equal(t, server.GetServerConf().GetRootConf().GetInt("rhTimeout"), 30, "rhTimeout")
	assert.Equal(t, server.GetServerConf().GetRootConf().GetInt("rTimeout"), 31, "rTimeout")
	assert.Equal(t, server.GetServerConf().GetRootConf().GetInt("wTimeout"), 32, "wTimeout")
