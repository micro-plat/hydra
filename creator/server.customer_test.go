...
func Test_newBaseBuilder(t *testing.T) {
	tests := []struct {
		name   string
		args   []interface{}
		repeat []interface{}
		...
		{name: "1. 没有入参,初始化空对象", args: []interface{}{}, want: BaseBuilder{"main": make(map[string]interface{})}},
		{name: "2. 单入参,初始化对象", args: []interface{}{"参数1"}, want: BaseBuilder{"main": "参数1"}},
		{name: "3. 多入参,初始化对象", args: []interface{}{"参数2", "参数1"}, want: BaseBuilder{"main": "参数2"}},
		{name: "4. 重复入参,初始化对象", args: []interface{}{"参数2"}, repeat: []interface{}{"参数1"}, want: BaseBuilder{"main": "参数1"}},
	}
	for _, tt := range tests {
		...
		assert.Equal(t, tt.want, got.httpBuilder.BaseBuilder, tt.name)
	}
}

func TestBaseBuilder_Sub(t *testing.T) {
	type args struct {
		name   string
		s      []interface{}
		repeat []interface{}
	...
	tests := []struct {
		...
		{name: "8. 入参-byte,构建自定义子节点", b: BaseBuilder{}, args: args{name: "x1", s: []interface{}{[]byte("10.1")}}, want: nil, wantErr: "配置：x1值类型不支持"},
		{name: "9. 入参-rune,构建自定义子节点", b: BaseBuilder{}, args: args{name: "x1", s: []interface{}{rune('1')}}, want: nil, wantErr: "配置：x1值类型不支持"},
		{name: "10. 重复入参-string,构建自定义子节点", b: BaseBuilder{}, args: args{name: "x1", s: []interface{}{"1111"}, repeat: []interface{}{"2222"}}, want: BaseBuilder{"x1": json.RawMessage([]byte("2222"))}, wantErr: ""},
	}
	for _, tt := range tests {
		func(name, wantErr string, b BaseBuilder) {
			defer func() {
