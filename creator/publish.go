...
func (c *conf) Pub(platName string, systemName string, clusterName string, registryAddr string, cover bool) error {
	...
	if proto == registry.FileSystem {
		return c.Encode2File(filepath.Join(registry.GetAddrs(registryAddr)[0], global.DefApp.LocalConfName), cover)
	}

	//创建注册中心，根据注册中心提供的接口进行配置发布
	r, err := registry.NewRegistry(registryAddr, global.DefApp.Log())
	if err != nil {
		return err
	}
