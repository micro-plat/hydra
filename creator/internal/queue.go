...
//Redis 添加Redis
func (c *Varqueue) Redis(nodeName string, address string, opts ...queueredis.Option) *Varqueue {
	return c.Custom(nodeName, queueredis.New(address, opts...))
}

//MQTT 添加MQTT
func (c *Varqueue) MQTT(nodeName string, address string, opts ...queuemqtt.Option) *Varqueue {
	return c.Custom(nodeName, queuemqtt.New(address, opts...))
}

//LMQ 添加本地内存作为消息队列
func (c *Varqueue) LMQ(nodeName string) *Varqueue {
	return c.Custom(nodeName, queuelmq.New())
}

//Custom 用户自定义消息队列
func (c *Varqueue) Custom(nodeName string, q interface{}) *Varqueue {
