...
//Redis 添加redis缓存
func (c *Varcache) Redis(nodeName string, address string, opts ...cacheredis.Option) *Varcache {
	return c.Custom(nodeName, cacheredis.New(address, opts...))
}

//GoCache 添加本地内存作为缓存
func (c *Varcache) GoCache(nodeName string, opts ...gocache.Option) *Varcache {
	return c.Custom(nodeName, gocache.New(opts...))
}

//Memcache 添加memcached作为缓存
func (c *Varcache) Memcache(nodeName string, addr string, opts ...memcached.Option) *Varcache {
	return c.Custom(nodeName, memcached.New(addr, opts...))
}

//Custom 自定义缓存配置
func (c *Varcache) Custom(nodeName string, q interface{}) *Varcache {
