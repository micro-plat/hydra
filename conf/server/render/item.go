...
//GetStatus 获取翻译后的状态码
func (r *RenderItem) GetStatus(input interface{}) (c string, err error) {
	return r.translate(r.Status, input)
}

//GetContent 获取翻译后的返回内容
func (r *RenderItem) GetContent(input interface{}) (c string, err error) {
	return r.translate(r.Content, input)
}

//Translate 翻译模板
func (r *RenderItem) translate(s string, input interface{}) (c string, err error) {
	if s == "" {
		return "", nil
	}
