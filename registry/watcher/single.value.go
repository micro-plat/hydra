...
//NewSingleWatcher 监控节点值发生变化
func NewSingleWatcher(r registry.IRegistry, path string, logger logger.ILogging) *SingleWatcher {
...
func (w *SingleWatcher) Start() (c chan *registry.NodeChangeArgs, err error) {
...
func (w *SingleWatcher) watch() (err error) {
LOOP:
...
//Close 关闭监控
func (w *SingleWatcher) Close() {
	w.done = true
	close(w.closeChan)
}
func (w *SingleWatcher) notifyDeleted() {
	w.mu.Lock()
	defer w.mu.Unlock()
...
func (w *SingleWatcher) notifyChanged(content []byte, version int32) {
	w.mu.Lock()
	defer w.mu.Unlock()
	op := registry.ADD
