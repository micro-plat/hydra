...
func NewChildrenWatcher(path string, deep int, registry registry.IRegistry, logger logger.ILogger) *ChildrenWatcher {
...
func (w *ChildrenWatcher) deleted() {
	...
	if v := atomic.LoadInt32(&w.changed); v > 0 && atomic.CompareAndSwapInt32(&w.changed, v, 0) {
		updater := NewCArgsByChange(DEL, w.deep, w.path, nil, 0)
		w.notify(updater)
	}
}
