 ...
 * @LastEditTime: 2021-09-23 17:05:02
 */
package dbr
...
func TestDBR_WatchChildren(t *testing.T) {
	tests := []struct {
		name     string
		provider string
		nodeType int //1.永久节点 2.临时节点  3.序列节点 4.物理不存在的节点
		...
		{name: "mysql-监控节点不存在", provider: "mysql", nodeType: 4, isDelete: false, isNotify: false},
		{name: "mysql-永久节点-监控被逻辑删除的节点", provider: "mysql", nodeType: 1, isDelete: true, isNotify: false},
		{name: "mysql-永久节点-正常节点监控", provider: "mysql", nodeType: 1, isDelete: false, isNotify: true},
		{name: "mysql-临时节点-监控被逻辑删除的节点", provider: "mysql", nodeType: 2, isDelete: true, isNotify: false},
		{name: "mysql-临时节点-正常节点监控", provider: "mysql", nodeType: 2, isDelete: false, isNotify: true},
		{name: "mysql-序列节点-监控被逻辑删除的节点", provider: "mysql", nodeType: 3, isDelete: true, isNotify: false},
		{name: "mysql-序列节点-正常节点监控", provider: "mysql", nodeType: 3, isDelete: false, isNotify: true},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			defer rgt.Close()
			data := fmt.Sprintf("%d", time.Now().UnixNano())
			path := fmt.Sprintf("/TestWatchChildren/%s", data)
			if tt.nodeType == 1 {
				err = rgt.CreatePersistentNode(path, data)
				assert.Equal(t, false, (err != nil), fmt.Sprintf("创建永久节点异常1,%v", err))
			}
			...
			for i := 0; i < 2; i++ {
				...
				if tt.nodeType == 1 {
					err = rgt.CreatePersistentNode(path1, data1)
					assert.Equal(t, false, (err != nil), fmt.Sprintf("创建永久节点异常2,%v", err))
				}
				if tt.nodeType == 2 {
					err = rgt.CreateTempNode(path1, data1)
					assert.Equal(t, false, (err != nil), fmt.Sprintf("创建临时节点异常2,%v", err))
				}
				if tt.nodeType == 3 {
					path1, err = rgt.CreateSeqNode(path1, data1)
					assert.Equal(t, false, (err != nil), fmt.Sprintf("创建序列节点异常2,%v", err))
				}
			}
