...
- #### ✓  六类服务器
   支持*API*、*WEB*、*WEBSOCKET*、*RPC*、*定时任务*、*消息消费* 等服务

- #### ✓ 跨平台服务
    支持windows,mac,linux等以服务方式安装、运行、停止、卸载等

- #### ✓ 多种部署方式
  支持三种部署方式：分布式集群部署、单机伪集群部署、单机部署
...
- #### ✓ 配置集中管理
    采用配置中心集中管理应用配置，应用本地零配置。支持的配置中心有zookeeper, redis, 本地等。

 - #### ✓ 配置热更新
    应用配置变更后自动生效，无须手动重启服务

- #### ✓ 服务注册与发现
    使用zookeeper, etcd, redis等作为注册中心，远程调用使用服务发现获得服务提供方式通过轮训等负载均衡方式进行远程调用

- #### ✓ 业务监控上报

    支持将metric信息(如：QPS、处理时长、响应等)等定时上报到influxdb，用于系统运行状况监控报表显示。
...
- #### ✓ 分布式日志归集
    ...
    使用[rlog](https://github.com/micro-plat/rlog)将分布式服务器日志提交到日志归集服务器，用于日志集中查询分析。

- #### ✓ 安全认证与访问控制
  
    支持白名单、黑名单访问控制，Basic Auth、API KEY、Cookie, JWT等安全认证。
...
- #### ✓ 提供常用组件库
    ...
    redis,memcached,数据库，mqtt,activeMQ,rpc,uuid,分布式锁等。
