...
func RoundRobin(service string, r naming.Resolver, log *logger.Logger, limits ...map[string]int) CustomerBalancer {
	l := make(map[string]int)
	if len(limits) > 0 {
		l = limits[0]
	}
	return &roundRobin{r: r, limiter: NewLimiter(service, l), Logger: log}
}
