...
func LocalFirst(service string, ip string, r naming.Resolver, limit map[string]int) CustomerBalancer {
	return &localFirst{r: r, ip: ip, limiter: NewLimiter(service, limit)}
}
...
func (rr *localFirst) watchAddrUpdates() error {
	...
	//检查是否有优先匹配项
	for _, v := range rr.addrs {
		rr.hasFirst = strings.HasPrefix(v.addr.Addr, rr.ip)
		if rr.hasFirst {
			break
		}
	}
...
func (rr *localFirst) Start(target string, config grpc.BalancerConfig) error {
	rr.mu.Lock()
	defer rr.mu.Unlock()
	if rr.done {
		return grpc.ErrClientConnClosing
...
// Get() calls.
func (rr *localFirst) Up(addr grpc.Address) func(error) {
	rr.mu.Lock()
	defer rr.mu.Unlock()
	var cnt int
...
func (rr *localFirst) down(addr grpc.Address, err error) {
	rr.mu.Lock()
	defer rr.mu.Unlock()
	for _, a := range rr.addrs {
		if addr == a.addr {
...
func (rr *localFirst) Get(ctx context.Context, opts grpc.BalancerGetOptions) (addr grpc.Address, put func(), err error) {
	var ch chan struct{}
	rr.mu.Lock()
	if rr.done {
		rr.mu.Unlock()
...
func (rr *localFirst) Notify() <-chan []grpc.Address {
	return rr.addrCh
}

func (rr *localFirst) Close() error {
	rr.mu.Lock()
	defer rr.mu.Unlock()
	rr.done = true
	if rr.w != nil {
