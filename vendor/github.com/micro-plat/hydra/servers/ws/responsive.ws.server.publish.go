...
func (w *WSServerResponsiveServer) publishCheck(data string) {
LOOP:
	for {
		select {
		case <-w.closeChan:
...
func (w *WSServerResponsiveServer) checkPubPath(data string) {
	w.pubLock.Lock()
...
//unpublish 删除已发布的节点
func (w *WSServerResponsiveServer) unpublish() {
	w.pubLock.Lock()
	defer w.pubLock.Unlock()
	for _, path := range w.pubs {
